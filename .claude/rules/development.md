...
`start-implementation` skillを経由せず、手動でブランチを作成したり、`docs/specs/`に仕様ドキュメントを作らずに実装を始めたりしない。ブランチの親をmainにすること自体は正しい手順(`start-implementation`も`origin/main`からブランチを作る)。禁止しているのは「skillを介さない手動での着手」であって、「mainを親にすること」ではない。
